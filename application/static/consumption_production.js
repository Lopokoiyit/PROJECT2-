...
function updateGraph(data, state, paths, productionLine, consumptionLine, leftAxis, rightAxis, yAxisLeft, yAxisRight, chartTitle) {
    const stateData = data.filter(row => row["state"] == state)
...
function createPlot(data, states) {
    ...
    var consumptionLine = d3.line()
        .x(d => x(d.financial_year))
        .y(d => yScaleRight(d.energy_consumption_pj))
    ...
    .on("click", function() {
        ...
        if (state !== chosenState) {
            ...
            updateGraph(data, chosenState, paths, productionLine, consumptionLine, leftAxis, rightAxis ,yAxisLeft, yAxisRight, chartTitle)          
        }
...
d3.json(url).then(function(energyData) {
    energyData.forEach(row => {
        row.energy_consumption_pj = +row.energy_consumption_pj;
